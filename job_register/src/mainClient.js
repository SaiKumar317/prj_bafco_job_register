...
const cbFunction = async (response) => {
  myResponse = response;
  sessionId = response.data.SessionId;
  companyId = response.data.CompanyId;
  accDate = response.data.AccountingDate;
  UserName = response.data.UserName;
  //  companyId to companyCode Convertion
  // var firstDigit = Math.round(companyId / 36 / 36);
  // var secondDigit = (companyId / 36).toString(16);
  // var thirdDigit = companyId % 36;
  // companyCode = (
  //   firstDigit.toString() +
  ...
  // Extracting components from the integer
  const year = Math.floor(accDate / 65536);
  const rawMonth = Math.floor((accDate % 65536) / 256); // 0-based month not assumed
  const rawDay = accDate % 256;

  const month = rawMonth.toString().padStart(2, "0"); // ensures MM format
  const day = rawDay.toString().padStart(2, "0"); // ensures DD format
  AccountDate = `${year}-${month}-${day}`;
  console.log("companyCode:", companyCode, " | companyId:", companyId);
  console.log("SessionId:", sessionId, typeof sessionId);
  console.log("response", response);
  console.log("AccountDate: ", AccountDate);
};

const settingBodyDetails = (response) => {};
const settingHeaderDetails = (response) => {};
